...
const resetEmblemsBtn = document.getElementById("resetEmblemsBtn");

const leagueEmblemInput = document.getElementById("leagueEmblemInput");
const div1EmblemInput = document.getElementById("div1EmblemInput");
const div2EmblemInput = document.getElementById("div2EmblemInput");
const div3EmblemInput = document.getElementById("div3EmblemInput");
const div4EmblemInput = document.getElementById("div4EmblemInput");

const leagueEmblemPreview = document.getElementById("leagueEmblemPreview");
const div1EmblemPreview = document.getElementById("div1EmblemPreview");
const div2EmblemPreview = document.getElementById("div2EmblemPreview");
const div3EmblemPreview = document.getElementById("div3EmblemPreview");
const div4EmblemPreview = document.getElementById("div4EmblemPreview");
...
function hydrateHeader(){
  ...
  leagueEmblemEl.src = state.leagueEmblem || DEFAULT_STATE.leagueEmblem;

  tabLogo1.src = state.divisions["1"].logo;
  tabLogo2.src = state.divisions["2"].logo;
  tabLogo3.src = state.divisions["3"].logo;
  tabLogo4.src = state.divisions["4"].logo;
}
...
function syncDivHeader(){
  ...
  divTitleEl.textContent = `Division ${d}`;
}
...
function bindAdmin(){
  ...
  leagueEmblemInput.addEventListener("change", ()=>handleEmblemPick("leagueEmblem", leagueEmblemInput, leagueEmblemPreview));
  div1EmblemInput.addEventListener("change", ()=>handleDivEmblemPick("1", div1EmblemInput, div1EmblemPreview));
  div2EmblemInput.addEventListener("change", ()=>handleDivEmblemPick("2", div2EmblemInput, div2EmblemPreview));
  div3EmblemInput.addEventListener("change", ()=>handleDivEmblemPick("3", div3EmblemInput, div3EmblemPreview));
  div4EmblemInput.addEventListener("change", ()=>handleDivEmblemPick("4", div4EmblemInput, div4EmblemPreview));

  resetEmblemsBtn.addEventListener("click", resetEmblems);
...
function openAdmin(){
  ...
  leagueEmblemPreview.src = state.leagueEmblem || DEFAULT_STATE.leagueEmblem;
  div1EmblemPreview.src = state.divisions["1"].logo;
  div2EmblemPreview.src = state.divisions["2"].logo;
  div3EmblemPreview.src = state.divisions["3"].logo;
  div4EmblemPreview.src = state.divisions["4"].logo;
...
function closeAdmin(){
  ...
  // clear file inputs
  leagueEmblemInput.value = "";
  div1EmblemInput.value = "";
  div2EmblemInput.value = "";
  div3EmblemInput.value = "";
  div4EmblemInput.value = "";
  teamLogoInput.value = "";
}
...
function handleEmblemPick(field, inputEl, previewEl){
...
function handleDivEmblemPick(divKey, inputEl, previewEl){
  const file = inputEl.files?.[0];
  if(!file) return;
  readFileAsDataUrl(file).then((dataUrl)=>{
    state.divisions[divKey].logo = dataUrl;
    previewEl.src = dataUrl;
    saveState();
    hydrateHeader();
    syncDivHeader();
...
function resetEmblems(){
  if(!confirm("エンブレムを初期状態に戻します。よろしいですか？")) return;
  state.leagueEmblem = DEFAULT_STATE.leagueEmblem;
  for(const k of ["1","2","3","4"]) state.divisions[k].logo = DEFAULT_STATE.divisions[k].logo;
  saveState();
  hydrateHeader();
  syncDivHeader();
  leagueEmblemPreview.src = state.leagueEmblem;
  div1EmblemPreview.src = state.divisions["1"].logo;
  div2EmblemPreview.src = state.divisions["2"].logo;
  div3EmblemPreview.src = state.divisions["3"].logo;
  div4EmblemPreview.src = state.divisions["4"].logo;
...
function renderAdminList(){
  ...
  teams.forEach(t=>{
    ...
    `;
    row.querySelector("button").addEventListener("click", (e)=>{ e.stopPropagation(); openEditTeam(t.id); });
...
function openEditTeam(teamId){
  if(adminModal.classList.contains("hidden")) openAdmin();
...
function handleLogoPick(){
  ...
  if(!file) return;
  readFileAsDataUrl(file).then((dataUrl)=>{ logoPreview.src = dataUrl; });
}
...
function addTeamToDiv(team, div){ state.divisions[String(div)].teams.push(team); }
...
function slug(name){
  return String(name).trim().toUpperCase().replace(/\s+/g,"_").replace(/[^A-Z0-9_]/g,"").slice(0,40) || "TEAM";
}
